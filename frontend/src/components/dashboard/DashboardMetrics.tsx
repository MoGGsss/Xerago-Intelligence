...
function MetricCard({
  ...
  return (
    <article className="rounded-xl border border-slate-200/90 bg-slate-50/70 p-4 transition-colors duration-200 hover:border-slate-300 hover:bg-slate-50 md:p-5">
  ...
  return (
    <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {Array.from({ length: 4 }).map((_, index) => (
        <div
          key={index}
          className="h-[98px] animate-pulse rounded-xl border border-slate-200 bg-slate-100"
        />
      ))}
    </section>
  )
